...
You need to adjust MODE by holding the button on the GRS Super Joystick until the button turns GREEN.  As indicated in the included manual, it's recommended to use 8-way mode for games like Ikari Warriors, T.N.K.III and Front Line.  Attach the 8-way joystick topper and hold down the WAY button + Button 1 until the LED on the WAY/MODE button is SOLID (not flashing/pulsing.)  Now assign in the `Keyboard` section of TS-RJConfigurator: Spinner Left to `Left Arrow` and Spinner Right to `Right Arrow` if you want to use the GRS Super Joystick as Player1.  Set Spinner Left to `C` and Spinner Right to `V` if you want to use the GRS Super Joystick as Player2 in MiSTer.  Now press the `Save Configuration` button and once it's finished, you can deattach the controller from the PC and attach it to MiSTer.  You need to assign your GRS game controller as Player1 or Player2 based on the previous configuration done in TS-RJConfigurator.
Next, you need to enable the core menu option: `SNAC & GRS Super Joystick > Use GRS Super JoyStick: ON`.  Now the rotate left and rotate right action will be assigned to the selected player.
When mapping the stick and buttons in the core, you should skip the button assignments for left and right rotation.
